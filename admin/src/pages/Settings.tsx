...
import { subscribeSettings, saveSettings } from "../services/adminFirestoreService";
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`px-5 py-2 rounded-lg text-[13px] font-semibold transition-all ${
              activeTab === tab ? "text-white shadow-sm" : "text-[#666] hover:text-[#111]"
            }`}
      ...
      <div className="bg-white rounded-xl border border-[#E5E5E5] shadow-sm p-6">
          ...
          <div className="space-y-6 max-w-2xl">
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { label: "Company Name", value: "Nesam Tours & Travels Private Limited" },
                  ...
                  <div key={f.label}>
                    <label className="text-[11px] font-semibold text-[#999] uppercase tracking-wide block mb-1.5">{f.label}</label>
                    <input
                ...
                <div className="md:col-span-2">
                  <label className="text-[11px] font-semibold text-[#999] uppercase tracking-wide block mb-1.5">Address</label>
                  <textarea
        ...
        {activeTab === "Booking" && (
          <div className="space-y-5 max-w-xl">
            ...
            {[
              { label: "Advance Booking Limit (days)", key: "advanceBookingLimit" },
              { label: "Minimum Booking Notice (hours)", key: "minBookingNotice" },
            ...
            <div>
              <label className="text-[11px] font-semibold text-[#999] uppercase tracking-wide block mb-1.5">Allow Cash on Delivery</label>
              <label className="flex items-center gap-3 cursor-pointer">
                <div className="relative">
                  <input type="checkbox" checked={!!formData.allowCod} onChange={(e) => setFormData({...formData, allowCod: e.target.checked})} className="sr-only peer" />
        ...
        {activeTab === "Notifications" && (
          <div className="space-y-4 max-w-xl">
            ...
            {[
              { label: "SMS Notifications", key: "smsEnabled", desc: "Send SMS for bookings and updates" },
              { label: "Email Notifications", key: "emailEnabled", desc: "Send email confirmations" },
              { label: "WhatsApp Notifications", key: "whatsappEnabled", desc: "Send WhatsApp updates via API" },
              { label: "Push Notifications", key: "pushEnabled", desc: "Browser and app push alerts" },
            ].map((n) => (
              <div key={n.label} className="flex items-center justify-between p-4 border border-[#E5E5E5] rounded-xl">
                <div>
                  <div className="text-[13px] font-semibold text-[#111]">{n.label}</div>
                  <div className="text-[11px] text-[#999] mt-0.5">{n.desc}</div>
                </div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <div className="relative">
                    <input type="checkbox" checked={!!formData[n.key]} onChange={(e) => setFormData({...formData, [n.key]: e.target.checked})} className="sr-only peer" />
                    <div className="w-10 h-5 bg-[#E5E5E5] rounded-full peer peer-checked:bg-[#E21B23] transition-colors" />
                    <div className="absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform peer-checked:translate-x-5" />
                  </div>
                </label>
        ...
        {activeTab === "Payment" && (
          <div className="space-y-5 max-w-xl">
            ...
            {[
              { label: "Razorpay Key ID", value: "rzp_live_XXXXXXXXXXXX" },
              { label: "Razorpay Key Secret", value: "••••••••••••••••" },
              { label: "UPI ID", value: "nesamtours@hdfc" },
            ].map((f) => (
              <div key={f.label}>
                <label className="text-[11px] font-semibold text-[#999] uppercase tracking-wide block mb-1.5">{f.label}</label>
                <input
