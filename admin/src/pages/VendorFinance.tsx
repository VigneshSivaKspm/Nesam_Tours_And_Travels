...
export default function VendorFinance() {
  const [activeTab, setActiveTab] = useState<"settlements" | "gst" | "razorpay">("settlements");
  ...
  const totalGrossRevenue = bookings.reduce((sum, b) => {
    ...
    const val = typeof b.fare === "number" ? b.fare : parseFloat(String(b.fare || "0").replace(/[^0-9.]/g, ""));
  ...
  const totalPayouts = payoutRequests
    ...
    .reduce((sum, p) => {
      const val = typeof p.amount === "number" ? p.amount : parseFloat(String(p.amount || "0").replace(/[^0-9.]/g, ""));
      return sum + (isNaN(val) ? 0 : val);
    }, 0);
  ...
  bookings.forEach((b) => {
    ...
    const fare = typeof b.fare === "number" ? b.fare : parseFloat(String(b.fare || "0").replace(/[^0-9.]/g, ""));
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Total Gross Revenue (Month)", value: `₹${Math.round(totalGrossRevenue).toLocaleString("en-IN")}`, color: "#E21B23" },
          { label: "Total Commission Earned", value: `₹${Math.round(totalCommission).toLocaleString("en-IN")}`, color: "#111" },
          { label: "Total Vendor Payouts", value: `₹${Math.round(totalPayouts).toLocaleString("en-IN")}`, color: "#10B981" },
          { label: "Pending Route Settlements", value: `₹${Math.round(pendingSettlements).toLocaleString("en-IN")}`, color: "#F59E0B" },
        ].map((s) => (
      ...
      {activeTab === "settlements" && (
        <div className="bg-white rounded-xl border border-[#E5E5E5] shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#E5E5E5]">
            <span className="text-[13px] font-bold text-[#111]">Vendor Financial Ledger</span>
            <button className="text-[12px] font-semibold px-4 py-2 rounded-lg border border-[#E5E5E5] text-[#E21B23] hover:bg-[#FEF2F2]">Export</button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1200px]">
              <thead>
                <tr className="bg-[#F9F9F9] border-b border-[#E5E5E5]">
                  {["Vendor", "Fleet", "City", "Commission", "Status", "Wallet Balance", "Actions"].map((h) => (
              ...
              <tbody>
                {liveVendors.length === 0 ? (
                  <tr className="border-b border-[#F5F5F5]">
                    <td colSpan={7} className="px-4 py-8 text-center text-[13px] text-[#999]">
                  ...
                  liveVendors.map((v, i) => (
                    <tr key={i} className="table-row border-b border-[#F5F5F5] last:border-0">
                      <td className="px-4 py-3">
                        <div className="text-[12px] font-semibold text-[#111]">{v.companyName || v.name}</div>
                        <div className="text-[10px] text-[#999]">{v.id}</div>
                      </td>
                      <td className="px-4 py-3 text-[12px] font-semibold text-[#111]">
                        {v.fleetSize !== undefined ? `${v.fleetSize} vehicles` : "0 vehicles"}
                      </td>
                      <td className="px-4 py-3 text-[12px] text-[#666]">{v.city || "—"}</td>
                      <td className="px-4 py-3 text-[11px]" style={{ color: "#E21B23" }}>
                        {v.commission ? `${v.commission}%` : "—"}
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-[12px] font-semibold text-green-700">{v.status || "Active"}</span>
                      </td>
                      <td className="px-4 py-3 text-[12px] font-semibold" style={{ color: "#111" }}>
                        {v.walletBalance !== undefined ? `₹${v.walletBalance.toLocaleString("en-IN")}` : (v.wallet || "₹0")}
                      </td>
                      <td className="px-4 py-3">
                        <button
                          onClick={() => alert(`Vendor Financial Details:\nVendor: ${v.companyName || v.name}\nWallet Balance: ${v.wallet || "₹0"}\nCommission Rate: ${v.commission || 10}%`)}
        ...
        <div className="space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div className="text-[12px] text-blue-800">
              <span className="font-semibold">GST Information (SAC 9964):</span> Passenger transport services are taxed at 5% GST without Input Tax Credit (ITC). GSTR-1 to be filed by the 11th of each month, and GSTR-3B by the 20th.
            </div>
          </div>
          <div className="bg-white rounded-xl border border-[#E5E5E5] shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-[#E5E5E5] flex items-center justify-between">
              <span className="text-[13px] font-bold text-[#111]">Monthly GST Summary</span>
              <button
                onClick={() => alert("GSTR-1 Format export generated and downloaded successfully.")}
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[700px]">
                <thead>
                  <tr className="bg-[#F9F9F9] border-b border-[#E5E5E5]">
                    {["Month", "Trip Revenue", "GST Rate", "GST Collected", "GST Payable", "Status", "Action"].map((h) => (
                      <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold text-[#999] uppercase tracking-wide whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {gstRecords.length === 0 ? (
                    ...
                    gstRecords.map((r, i) => (
                      <tr key={i} className="table-row border-b border-[#F5F5F5] last:border-0">
                        ...
                        <td className="px-4 py-3 text-[12px] text-[#444]">{r.totalTripRevenue}</td>
                        <td className="px-4 py-3 text-[12px] font-semibold" style={{ color: "#E21B23" }}>{r.gstRate}</td>
                        <td className="px-4 py-3 text-[13px] font-bold text-[#111]">{r.gstCollected}</td>
                        <td className="px-4 py-3 text-[13px] font-bold text-[#111]">{r.gstPayable}</td>
                        <td className="px-4 py-3">
                          <span className={`text-[11px] font-semibold px-2 py-0.5 rounded ${r.status === "Filed" ? "text-green-700 bg-green-50" : "text-yellow-700 bg-yellow-50"}`}>{r.status}</span>
                        </td>
                        <td className="px-4 py-3">
                          <button
                            onClick={() => alert(`Downloading GST report for ${r.month}`)}
                            className="text-[11px] px-2 py-1 rounded-md border border-[#E5E5E5] hover:bg-[#FEF2F2] text-[#E21B23] font-medium transition-colors cursor-pointer"
                          >
                            Download
                          </button>
      ...
      {activeTab === "razorpay" && (
        <div className="space-y-4">
          <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 flex items-start gap-3">
            <svg className="w-5 h-5 text-purple-600 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
            <div className="text-[12px] text-purple-800">
              <span className="font-semibold">Razorpay Route (Automated Splits):</span> Customer payments are instantly split between the platform commission account and vendor linked accounts via Razorpay Route.
            </div>
          </div>
          <div className="bg-white rounded-xl border border-[#E5E5E5] shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-[#E5E5E5] flex items-center justify-between">
              <span className="text-[13px] font-bold text-[#111]">Razorpay Route Settlements</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[800px]">
                <thead>
                  <tr className="bg-[#F9F9F9] border-b border-[#E5E5E5]">
                    {["Transfer ID", "Booking ID", "Customer", "Amount", "Method", "Date", "Status"].map((h) => (
                      <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold text-[#999] uppercase tracking-wide whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {payments.length === 0 ? (
                    ...
                    payments.map((p, i) => (
                      <tr key={i} className="table-row border-b border-[#F5F5F5] last:border-0">
                        ...
                        <td className="px-4 py-3">
                          <span className={`text-[11px] font-semibold px-2 py-0.5 rounded ${razorpayStyle[p.status] || "text-green-700 bg-green-50"}`}>{p.status}</span>
                        </td>
                      </tr>
                    ))
                  )}
