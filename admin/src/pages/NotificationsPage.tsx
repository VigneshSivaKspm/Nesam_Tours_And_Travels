...
export default function Notifications() {
  ...
  useEffect(() => {
    ...
    const unsub2 = subscribeCustomers((custs) =>
      setUsers((prev) => [
    ...
    const unsub3 = subscribeDrivers((drvs) =>
      setUsers((prev) => [
    ...
    const unsub4 = subscribeVendors((vends) =>
      setUsers((prev) => [
  ...
  const markAllRead = () => {
    ...
    notifs.forEach((n) =>
      setFirestoreDocument(COLLECTIONS.NOTIFICATIONS, String(n.id), {
        read: true,
      }),
    );
  };
  ...
  const handleSend = () => {
    ...
    if (recipientId === "all") {
      newNotif.broadcast = true;
    } else {
      newNotif.recipientId = recipientId;
    }
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            value: notifs.filter((n) => n.type === "marketplace").length,
            color: "#8B5CF6",
          },
        ].map((s) => (
          <div
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[12px] text-[#999] mt-0.5">
            Automated system notifications, alerts, SMS/WhatsApp broad-casts
          </p>
        </div>
      ...
      {activeTab === "inbox" && (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2 bg-white rounded-xl border border-[#E5E5E5] shadow-sm p-4">
            ...
            <button
              onClick={markAllRead}
              className="ml-auto text-[11px] font-semibold text-[#666] hover:text-[#111] transition-colors px-3 py-1 border border-[#E5E5E5] rounded-lg cursor-pointer"
            >
              Mark all read
            </button>
          </div>
          ...
          <div className="bg-white rounded-xl border border-[#E5E5E5] shadow-sm overflow-hidden">
            ...
            {filtered.length === 0 && (
              <div className="py-12 flex flex-col items-center text-center">
                <svg
                  className="w-10 h-10 text-[#E5E5E5] mb-3"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
      ...
      {activeTab === "send" && (
        <div className="max-w-2xl">
          <div className="bg-white rounded-xl border border-[#E5E5E5] shadow-sm p-6 space-y-5">
            ...
            <div>
              <label className="text-[11px] font-semibold text-[#999] uppercase tracking-wide block mb-2">
                Recipient
              </label>
              <select
                value={recipientId}
            ...
            <div>
              ...
              />
              <div className="text-[10px] text-[#999] mt-1 text-right">
                {sendMessage.length} / 160 chars
                {sendChannel === "SMS" ? " (SMS limit)" : ""}
              </div>
            </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            {[
              {
                channel: "SMS",
                detail: "0.50₹/message via configured gateway",
                icon: "M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z",
              },
              {
                channel: "WhatsApp",
                detail: "₹1–2/conversation via WhatsApp Business API",
                icon: "M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z",
              },
              {
                channel: "Email",
                detail: "Via SMTP — Booking confirmations & invoices",
                icon: "M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z",
              },
              {
                channel: "Push",
                detail: "Firebase FCM — App & browser push alerts",
                icon: "M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9",
              },
            ].map((c) => (
              <div
                key={c.channel}
                className="bg-white rounded-xl border border-[#E5E5E5] p-3 flex items-start gap-3"
              >
