...
import { subscribeBookings, updateFirestoreDocument, setFirestoreDocument, COLLECTIONS, subscribeDrivers } from "../services/adminFirestoreService";

const getStatusStyle = (status: string) => {
  switch (status) {
    case "Completed":
      return { background: "#F0FDF4", color: "#16A34A", borderColor: "#86EFAC" };
    case "Ongoing":
    ...
    case "Cancelled":
      return { background: "#FEF2F2", color: "#E21B23", borderColor: "#FECACA" };
    default:
      return { background: "#F5F5F5", color: "#666666", borderColor: "#E5E5E5" };
  }
};

const getTimeline = (status: string, driver: string) => {
  const isCancelled = status === "Cancelled";
  const isConfirmedDone = ["Confirmed", "Assigned", "Ongoing", "Completed"].includes(status);
  const isDriverAssignedDone = (!!driver && driver !== "—" && driver !== "Not Assigned") || ["Assigned", "Ongoing", "Completed"].includes(status);
  ...
  if (isCancelled) {
    return [
      { step: "Booking Created", time: "24 Aug 2024, 05:15 AM", done: true, cancelled: false },
      { step: "Trip Cancelled", time: "24 Aug 2024, 05:30 AM", done: true, cancelled: true },
    ];
  }

  return [
    { step: "Booking Created", time: "24 Aug 2024, 05:15 AM", done: true, cancelled: false },
    { step: "Confirmed", time: "24 Aug 2024, 05:17 AM", done: isConfirmedDone, cancelled: false },
    { step: "Driver Assigned", time: isDriverAssignedDone ? "24 Aug 2024, 05:45 AM" : "—", done: isDriverAssignedDone, cancelled: false },
    { step: "Driver Arrived", time: isArrivedDone ? "24 Aug 2024, 06:25 AM" : "—", done: isArrivedDone, cancelled: false },
    { step: "Trip Started", time: isStartedDone ? "24 Aug 2024, 06:30 AM" : "—", done: isStartedDone, cancelled: false },
    { step: "Trip Completed", time: isCompletedDone ? "24 Aug 2024, 07:15 AM" : "—", done: isCompletedDone, cancelled: false },
  ];
};

// Fare rules per service + vehicle (base fare, per-km rate, driver batta, toll)
const fareConfig: Record<string, { base: number; perKm: number; batta: number; gstRate: number }> = {
  "Airport Taxi_Sedan":          { base: 350, perKm: 12, batta: 250, gstRate: 0.05 },
  "Airport Taxi_Toyota Innova":  { base: 450, perKm: 18, batta: 350, gstRate: 0.05 },
  "Airport Taxi_Toyota Etios":   { base: 350, perKm: 11, batta: 250, gstRate: 0.05 },
  "Airport Taxi_Maruti Dzire":   { base: 350, perKm: 12, batta: 250, gstRate: 0.05 },
  "Outstation Cab_Toyota Innova Crysta": { base: 0, perKm: 22, batta: 400, gstRate: 0.05 },
  "Outstation Cab_Toyota Innova": { base: 0, perKm: 20, batta: 400, gstRate: 0.05 },
  "Outstation Cab_Maruti Ertiga": { base: 0, perKm: 14, batta: 300, gstRate: 0.05 },
  "One Way Taxi_Maruti Dzire":   { base: 0, perKm: 12, batta: 300, gstRate: 0.05 },
  "One Way Taxi_Maruti Ertiga":  { base: 0, perKm: 14, batta: 300, gstRate: 0.05 },
  "Local Rental_Honda City":     { base: 800, perKm: 12, batta: 150, gstRate: 0.05 },
  "Tour Package_Tempo Traveller":{ base: 0, perKm: 28, batta: 500, gstRate: 0.05 },
};
...
function computeFareBreakdown(
  ...
  const baseFare = Math.round(preTaxTotal - distCharge - battaCharge - toll + discount);
...
export default function BookingDetails({ bookingId, onBack }: { bookingId: string; onBack: () => void }) {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    await setFirestoreDocument(COLLECTIONS.BOOKINGS, booking.id, { status: newStatus });
    updateFirestoreDocument(COLLECTIONS.BOOKINGS, booking.id, { status: newStatus });
  ...
  const handleCancelTrip = async () => {
    ...
    if (window.confirm("Are you sure you want to cancel this trip?")) {
      ...
      await setFirestoreDocument(COLLECTIONS.BOOKINGS, booking.id, { status: "Cancelled" });
      updateFirestoreDocument(COLLECTIONS.BOOKINGS, booking.id, { status: "Cancelled" });
  ...
  const handleReassignDriver = async () => {
    ...
    setLocalOverride((prev) => ({ ...prev, driver: assignDriver, status: newStatus }));
    await setFirestoreDocument(COLLECTIONS.BOOKINGS, booking.id, { driver: assignDriver, status: newStatus });
    updateFirestoreDocument(COLLECTIONS.BOOKINGS, booking.id, { driver: assignDriver, status: newStatus });
    setShowAssignModal(false);
    showNotification(`Driver reassigned to ${assignDriver} - Status updated to Confirmed`);
  };
  ...
  const fare = computeFareBreakdown(totalFare, booking.service, booking.vehicle, distanceKm, toll, discount);
  const recomputed = Math.round((fare.baseFare + fare.distCharge + fare.battaCharge + fare.toll - fare.discount) * (1 + fare.gstRate));
  const verified = recomputed === totalFare;

  const timelineItems = getTimeline(booking.status || "Confirmed", booking.driver || "");
  ...
  return (
    <div className="p-6 space-y-5 relative">
      ...
      <div className="flex items-center gap-4">
        ...
        <span className="text-[13px] font-mono font-bold" style={{ color: "#E21B23" }}>{booking.id}</span>
        <div className="ml-auto flex gap-2">
          <button
            onClick={() => {
              const newStatus = prompt("Enter new status (Confirmed, Assigned, Ongoing, Completed, Cancelled):", booking.status);
          ...
          <button onClick={() => setShowAssignModal(true)} className="px-4 py-2 text-[12px] font-semibold border border-[#E5E5E5] rounded-lg hover:bg-[#F5F5F5] text-[#444] transition-colors">
            {booking.driver && booking.driver !== "—" && booking.driver !== "Not Assigned" ? "Reassign Driver" : "Assign Driver"}
          </button>
          <button
            onClick={async () => {
              const docId = `INV-${booking.id}`;
              const invNo = `NES-INV-${new Date().getFullYear()}-${booking.id.replace(/[^0-9]/g, "").slice(-4) || "0001"}`;
              const parseNum = (v: any) => parseFloat((v || "0").toString().replace(/[^0-9.]/g, "")) || 0;
              ...
              const payload = {
                ...
                items: [
                  { description: `${booking.service} (${booking.pickup} → ${booking.drop})`, amount: subtotal }
                ],
                subtotal,
                discount: 0,
              ...
              showNotification(`Tax invoice ${invNo} generated & available in Finance > Invoices`);
            }}
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-xl border border-[#E5E5E5] shadow-sm p-5">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                <div key={f.label}>
                  <div className="text-[10px] font-semibold text-[#999] uppercase tracking-wide mb-1">{f.label}</div>
                  <div className="text-[13px] font-medium text-[#111]">{f.value}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-[#E5E5E5] shadow-sm p-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Driver Name", value: !booking.driver || booking.driver === "—" ? "Not Assigned" : booking.driver },
                { label: "Phone", value: !booking.driver || booking.driver === "—" ? "—" : "+91 94567 12345" },
                { label: "Rating", value: !booking.driver || booking.driver === "—" ? "—" : "4.8 ★" },
                { label: "Status", value: !booking.driver || booking.driver === "—" ? "Unassigned" : (booking.status === "Completed" ? "Completed" : "On Trip") },
              ].map((f) => (
                <div key={f.label}>
                  <div className="text-[10px] font-semibold text-[#999] uppercase tracking-wide mb-1">{f.label}</div>
                  <div className="text-[13px] font-medium text-[#111]">{f.value}</div>
              ...
              <div className="space-y-5">
                {timelineItems.map((t, i) => (
                  <div key={i} className="flex items-start gap-4">
                    ...
                    <div>
                      <div className={`text-[13px] font-semibold ${t.cancelled ? "text-[#E21B23]" : t.done ? "text-[#111]" : "text-[#999]"}`}>
                        {t.step}
                      </div>
                      <div className="text-[11px] text-[#999] mt-0.5">{t.time}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-xl border border-[#E5E5E5] shadow-sm p-5">
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-[12px]">
                ...
                <span className="font-medium text-[#111]">{fmt(fare.baseFare)}</span>
              </div>
              <div className="flex justify-between text-[12px]">
                <span className="text-[#666]">Distance Charge ({distanceKm} km)</span>
                <span className="font-medium text-[#111]">{fmt(fare.distCharge)}</span>
              </div>
              <div className="flex justify-between text-[12px]">
                <span className="text-[#666]">Driver Batta</span>
                <span className="font-medium text-[#111]">{fmt(fare.battaCharge)}</span>
              </div>
              <div className="flex justify-between text-[12px]">
              ...
              <div className="flex justify-between text-[12px]">
                ...
                <span className="font-medium text-[#111]">{fmt(fare.toll)}</span>
              </div>
              {fare.discount > 0 && (
                <div className="flex justify-between text-[12px]">
                  <span className="text-green-600">Discount Applied</span>
                  <span className="font-medium text-green-600">−{fmt(fare.discount)}</span>
                </div>
              )}
              ...
              <div className="border-t border-dashed border-[#E5E5E5] pt-3 space-y-2">
                <div className="flex justify-between text-[12px]">
                  ...
                  <span className="font-medium text-[#999]">{fmt(fare.preTaxTotal)}</span>
                </div>
                <div className="flex justify-between text-[12px]">
                  <span className="text-[#666]">
                    GST ({(fare.gstRate * 100).toFixed(0)}%)
                    <span className="text-[10px] ml-1 text-[#999]">CGST 2.5% + SGST 2.5%</span>
                  </span>
                  <span className="font-medium text-[#111]">{fmt(fare.gst)}</span>
                </div>
              </div>

              {/* Grand Total */}
              <div className="border-t-2 border-[#111] pt-3 flex justify-between items-center">
                <span className="text-[14px] font-bold text-[#111]">Total Amount</span>
                <span className="text-[16px] font-bold" style={{ color: "#E21B23" }}>{booking.fare}</span>
              </div>
            ...
            <div className="mt-4 pt-4 border-t border-[#E5E5E5]">
              ...
              <div className="flex justify-between text-[12px]">
                ...
                <span className={`font-semibold ${booking.payment === "Paid" ? "text-green-600" : "text-[#E21B23]"}`}>{booking.payment}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-[#E5E5E5] shadow-sm p-5 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              >
                {["Pending", "Confirmed", "Assigned", "Ongoing", "Completed", "Cancelled"].map((st) => (
            ...
            <div className="grid grid-cols-2 gap-3 pt-1">
              ...
              <button
                ...
                style={{ background: "#E21B23" }}
              >
                {booking.driver && booking.driver !== "—" && booking.driver !== "Not Assigned" ? "Reassign" : "Assign"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b pb-3">
              ...
              <button onClick={() => setShowAssignModal(false)} className="text-gray-400 hover:text-gray-700">✕</button>
            </div>
            <div className="space-y-3">
              <label className="block text-xs font-medium text-gray-700">Select Available Driver:</label>
              <select className="w-full p-2.5 border rounded-lg text-xs font-medium bg-white focus:border-[#E21B23] focus:outline-none" onChange={(e) => setAssignDriver(e.target.value)} value={assignDriver}>
                <option value="">Select a driver...</option>
                {availableDrivers.map((d: any) => (
                  <option key={d.id || d.name} value={d.name}>
                    {d.name} {d.vehicle ? `(${d.vehicle})` : ''} {d.status ? ` - ${d.status}` : ''}
                  </option>
                ))}
              </select>
              <div className="flex gap-2 pt-2">
                ...
                <button
                  ...
                  className={`w-1/2 p-2 text-white text-xs font-bold rounded-lg transition-opacity ${
                    assignDriver ? "bg-[#E21B23] hover:opacity-90 cursor-pointer" : "bg-gray-300 cursor-not-allowed"
                  }`}
                >
                  Confirm
                </button>
