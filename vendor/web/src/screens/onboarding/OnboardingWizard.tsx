...
function initialStep(record: VendorRecord | null): number {
  ...
  if (
    ...
    const first = ONBOARDING_STEPS.findIndex(
      (s) => s.key === record.review!.flaggedSections[0],
    );
...
export const OnboardingWizard: React.FC<OnboardingWizardProps> = ({
  ...
  if (prefillLoading)
    return <FullScreenLoader label="Preparing your application…" />;
        ...
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <img
              ...
              className="w-8 h-8 rounded-lg object-contain bg-white border border-[#E5E5E5] p-1 shadow-sm"
            />
            <div>
          ...
          <button
            onClick={onSignOut}
            className="flex items-center gap-1.5 text-[12px] font-semibold text-[#666] hover:text-[#111]"
          >
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        >
          {ONBOARDING_STEPS.map((s, i) => {
            ...
            const active = i === step;
            const flagged = correction?.review?.flaggedSections.includes(
              s.key as never,
            );
            return (
              <li key={s.key} className="flex-1 min-w-0">
                ...
                >
                  ...
                  <span
                    className={`text-[10px] sm:text-[11px] font-semibold truncate max-w-full ${active ? "text-[#111]" : "text-[#999]"}`}
                  >
                    {s.short}
                  </span>
                </button>
        ...
        {notice && step === 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-[12px] font-medium text-blue-900">
            {notice}
          </div>
        )}
            ...
            <div className="text-[12px] text-amber-900 space-y-1">
              <div className="font-bold text-[13px]">
                {correction.status === "REJECTED"
                  ...
                  : "The Nesam team requested changes"}
              </div>
              {correction.review?.note && (
                <p className="whitespace-pre-wrap">{correction.review.note}</p>
              )}
              {!!correction.review?.flaggedSections.length && (
                <p>
                  Please update:{" "}
                  <span className="font-semibold">
                    {correction.review.flaggedSections
        ...
        <section className="bg-white rounded-2xl border border-[#E5E5E5] shadow-sm p-5 sm:p-6">
          <div className="mb-5">
            ...
            <h1 className="text-[17px] font-bold text-[#111] mt-0.5">
              {current.title}
            </h1>
          </div>
          {renderStep()}
        ...
        <p className="text-center text-[11px] text-[#BBB]">
          Your progress is saved after each step — you can sign out and continue
          later.
        </p>
      </main>
    </div>
